...
namespace Physics {
	...
	void GridMover::Update( double dt ) {
		...
		// Check if last update stop us in the next point or which way (if any) we need to move on x or y
		//////////////////////////////////////////////////////////////////////////
		// Left or right
		//////////////////////////////////////////////////////////////////////////
		...
		if ((leftright == 0) && (updown == 0)) {
			Point prevPoint = this->currentPoint; // Useful for debugging later
			if (this->moving) { // Start the sequence
				this->moving = false;
				this->currentPoint = this->nextPoint; // We arrived at the next point, update current point to reflected that
				this->preaction = true;
			}

			if (this->preaction || this->postaction) {
				unsigned int time;
				if (preaction) {
					time = TIMING::POINT_ENTER;
				}
				else {
					time = TIMING::POINT_LEAVE;
				}
				// Check if there is an action upon entering the point
				// Reset the delay in ActionComplete
				if (this->actionHandlers.find(time) != this->actionHandlers.end()) {
					this->preaction = false;
					unsigned int enterActionID = this->actionHandlers[time];
					...
					if (this->pointHandlerdata.find(this->currentPoint) != this->pointHandlerdata.end()) {
						...
						if (time == TIMING::POINT_LEAVE) {
							this->pointHandlerdata.erase(prevPoint);
						}
					}

					this->msgrouter->SendSP(actionEnv);
				}
				else { // If no action handler continue the sequence
					if (this->preaction) {
						this->preaction = false;
						this->postaction = true;
					}
					else if (this->postaction) {
						this->postaction = false;
					}
				}
			}
				...
				if ((this->currentPoint.x == this->path->PeekNextPoint().x) && (this->currentPoint.y == this->path->PeekNextPoint().y)) {
					...
					if ((abs(nextPeek.x - this->currentPoint.x) <= 1) && (abs(nextPeek.y - this->currentPoint.y) <= 1)) { // Movement will be 1 away so still good
						...
						this->nextPoint = prevPoint; // Peek what the next point is after that, and store it in nextPoint
					}
				}
				this->moving = true;
			}
