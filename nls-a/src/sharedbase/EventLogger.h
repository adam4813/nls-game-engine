...
#define LOG(x, y) ::EventLogger::GetEventLogger()->LogToDisk((x), (y), __FILE__, __LINE__, __FUNCTION__); // Original logger
#define LOG2(x, y) {if (this->debug_output) {::EventLogger::GetEventLogger()->LogToDisk((x), (y), __FILE__, __LINE__, __FUNCTION__);}} // Uses a member variable to determine output
