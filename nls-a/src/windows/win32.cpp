...
boost::any win32::CreateGUIWindow( int width, int height, std::string title, WINDOW_FLAGS flags ) {
	...
	hwnd = CreateWindowEx(NULL, wc.lpszClassName, title.c_str(), WS_OVERLAPPEDWINDOW | CS_HREDRAW, 10, 10, width, height, NULL, NULL, wc.hInstance, NULL);
	if (!hwnd) {
		return 0;
	}
	UpdateWindow(hwnd);
...
void win32::RouteMessages() {
	while (PeekMessage(&this->msg, NULL, 0, 0, PM_REMOVE) != 0) {
...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
std::string win32::GetPath(DIRS dir_id) {
	int csidl;
