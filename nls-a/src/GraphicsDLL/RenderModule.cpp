		...
		BOOST_FOREACH(RenderableComponent * component, this->renderableComponents) {
			if (component->GetCoordSpace() == space) {
				...
				if (fabs(scale) <= 0.000001f) { // 0 (or way too close to it) is not a valid scale.  Negative is fine though!
					char temp[57]; // 57 is the count of numerals in the number computed from pow(2, 64) + 1 (The +1 is for the null terminator.)
					sprintf_s(temp, "%u", owner->GetId());
					LOG(LOG_PRIORITY::ERR, "Entity '" + std::string(temp) + "' has an invalid calculated 'world' scale: scale must be non-zero and positive!");
					continue;
				}
